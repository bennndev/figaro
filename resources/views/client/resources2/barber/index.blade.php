<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto">
            <form method="GET" action="{{ route('client.barbers.index') }}" 
                  class="mb-6 bg-[#2A2A2A] text-white p-6 rounded shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4">
                {{-- Nombre --}}
                <div>
                    <label for="name" class="block text-sm font-medium">Nombre</label>
                    <input type="text" name="name" id="name" value="{{ $filters['name'] ?? '' }}"
                        class="mt-1 block w-full border-gray-500 rounded-md bg-[#1E1E1E] text-white focus:ring-indigo-500 focus:border-indigo-500">
                </div>
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse ($barbers as $barber)
            <div class="bg-[#2A2A2A] text-white shadow-md rounded-lg overflow-hidden p-6">
                <h3 class="text-xl font-bold">{{ $barber->name }} {{ $barber->last_name }}</h3>
                <p class="text-gray-300 mt-2">{{ Str::limit($barber->description, 100) }}</p>

                {{-- Especialidades --}}
                <div class="mt-2">
                    <p class="text-sm font-semibold text-white">Especialidades:</p>
                    <div class="flex flex-wrap gap-1 mt-1">
                        @forelse($barber->specialties as $specialty)
                            <span class="bg-[#1E1E1E] text-gray-300 text-xs px-2 py-1 rounded">
                                {{ $specialty->name }}
                            </span>
                        @empty
                            <span class="text-xs text-gray-400">Sin especialidades registradas</span>
                        @endforelse
                    </div>
                </div>

                <a href="{{ route('client.barbers.show', $barber->id) }}"
                   class="text-indigo-400 hover:underline mt-4 inline-block">Ver más</a>
            </div>
        @empty
            <p class="text-gray-400">No hay barberos disponibles en este momento.</p>
        @endforelse
    </div>
